...
function request_help_with_GUM() {
    chrome.runtime.sendMessage({
            'method': 'AP_help_with_GUM',
            'primaryPattern': location.protocol + '//' + location.host + '/*'
        },
        function (response) {
            if (response) {
...
function update_devices(sinkNo = 0) {
    sink_no = sinkNo;
    get_devices();
}
...
function inspect_devices(deviceInfos) {
    ...
    for (var i = 0; i != deviceInfos.length; i++) {
        ...
        deviceInfo.kind == 'audiooutput' && log('  Devices[' + i + ']: ' + deviceInfo.kind + ': ' + deviceInfo.label + ': ' + deviceInfo.deviceId)
    ...
    findOk || (sink_no && log('target audio device not found:' + sink_no));
    with_or_without_GUM();
}
